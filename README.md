...
- **Inbox JSON Export & Search API** — programmatic message text querying (`search_messages`), recent message retrieval (`get_recent_messages`), unread filtering (`get_unread_messages`), type filtering (`get_messages_by_type`), sender volume counting (`count_messages_by_sender`), and structured dictionary export (`export_inbox_to_dict`)
...
CLI (click)
 └─ MuninnApp (textual)
     ...
     └─ MainScreen          ← primary interface
         ...
         └─ Services
             ├─ MessageStore     ← load, filter, unread counters, & search messages by agent
